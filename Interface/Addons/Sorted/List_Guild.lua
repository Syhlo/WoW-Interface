...
local function Button_OnUpdate(self)
    ...
    local hasItem, hasCooldown, repairCost, speciesID, level, breedQuality, maxHealth, power, speed, name = GameTooltip:SetGuildBankItem(self.bag, self.slot)
    if not hasItem then
        GameTooltip:SetHyperlink(link)
    elseif ( speciesID and speciesID > 0 ) then
...
function S.CreateGuildItemList(parent)
    local list =  S.CreateList(parent, CreateEntry, S.ItemColumns, "itemColumnSettings", true, S.ItemGroups, S.itemGroupingSettings)
    list:ClearAllPoints()
    ...
    function list.freeSpace:Update()
        ...
        if data.guild then
            ...
            local max = 0
            if guildTab then
                if data.guild and data.guild.tabs[guildTab].canView and data.guild.tabs[i].count then
