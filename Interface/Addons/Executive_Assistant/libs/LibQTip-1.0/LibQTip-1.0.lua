local MAJOR = "LibQTip-1.0moddedForExecAssist" -- search "ExecAssist Adjustment" for changes
...
-- ExecAssist Adjustment: EA needs the remaining width dumped into the last cell of the col-span for esthetic reasons
--  As https://www.wowace.com/projects/libqtip-1-0/issues/35 has not yet been addressed or and the max-cell-size parameter 
--  not being obeyed (like it should) two adjustments needs to be made: 
--	REMed			width = width / (right - left + 1)
--  This forces maxNeedWidthPerCol=width rather a percentage
--  Later, instead of spreading 100% of the needed width to all spanned cells (left, right) it's dumped into the right-most cell 
-- (right, right). It's done like to create the least impact on the existing lib code

                if width > maxNeedWidthPerCol then  -- NB, maxNeedWidthPerCol now = width
        ...
        if maxNeedCols then
            ...
            for col = right, right do -- ExecAssist Adjustment: was left, right
