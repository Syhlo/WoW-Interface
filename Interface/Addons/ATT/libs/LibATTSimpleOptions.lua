...
local function create_MenuButton(name, parent)
	...
	local function button_OnClick(self)
		...
		if ( playSound ) then
			PlaySound(SOUNDKIT.U_CHAT_SCROLLSOL_BUTTON);
		end
	end
...
do
	...
	local function paneLSOL_okay(self)
		for control in pairs(self.controls) do
	...
	local function paneLSOL_default(self)
		for control in pairs(self.controls) do
	...
	local function makePanel(name, parentName, controlCreationFunc)
		...
		if not parentName then
			...
			panel = CreateFrame("Frame", parentName .. "_PaneLSOL_" .. name)
		end
		panels[panel] = true
		...
		panel.okay = paneLSOL_okay
		panel.cancel = paneLSOL_cancel
		panel.default = paneLSOL_default

		InterfaceOptions_AddCategory(panel)

		panel.controlCreationFunc = controlCreationFunc
		panel:SetScript("OnShow", paneLSOL_OnShow)
...
do
	...
	function panelMeta:MakeButton(...)
		...
		if args.newsize then button:SetWidth(110); button:SetHeight(30); 
		else button:SetWidth(90); button:SetHeight(28) 
		end
...
do
	...
	function panelMeta:MakeToggle(...)
	    ...
	    toggle.Text:SetFont(GameFontNormal:GetFont(), 11)
